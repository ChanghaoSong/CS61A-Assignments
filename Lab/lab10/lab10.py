...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            evaluated_args=operands.map(calc_eval)
            fn=calc_eval(operator)
            return calc_apply(fn, evaluated_args) # UPDATE THIS FOR Q2, what is type(operator)?
...
def eval_and(expressions):
    ...
    while cur is not nil:
        combined_val=calc_eval(cur.first)

        if combined_val is scheme_f:
            return scheme_f
